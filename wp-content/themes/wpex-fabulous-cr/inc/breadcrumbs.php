...
if ( !function_exists( 'wpex_display_breadcrumbs' ) ) {
	...
	function wpex_display_breadcrumbs() {
		...
		//if ( !is_singular( 'post' ) && !is_singular( 'research_post' ) && !is_tax() ) return;
        if ( is_home() ) return;
...
if ( !function_exists( 'wpex_breadcrumbs' ) ) {
	function wpex_breadcrumbs( $args = array() ) {
        ...
        // It was using extract to get the array values, but VIP Scanner says it's a bad practice
        // So we have to parse the vars manually
        $parsed_args = wp_parse_args( $args, $defaults );
        if( array_key_exists( 'show_home', $parsed_args ) ) $show_home = $parsed_args['show_home'];
        if( array_key_exists( 'front_page', $parsed_args ) ) $front_page = $parsed_args['front_page'];
        if( array_key_exists( 'separator', $parsed_args ) ) $separator = $parsed_args['separator'];
        if( array_key_exists( 'before', $parsed_args ) ) $before = $parsed_args['before'];
        if( array_key_exists( 'after', $parsed_args ) ) $after = $parsed_args['after'];
        if( array_key_exists( 'echo', $parsed_args ) ) $echo = $parsed_args['echo'];
        if( array_key_exists( 'show_posts_page', $parsed_args ) ) $show_posts_page = $parsed_args['show_posts_page'];
        if( array_key_exists( 'singular_post_taxonomy', $parsed_args ) ) $singular_post_taxonomy = $parsed_args['singular_post_taxonomy'];
		...
		elseif ( is_home() ) {
            ...
            // Change deprecated get_page function for get_post
		...
		elseif ( is_singular() ) {
			...
			if ( 'page' !== $post_type && 'post' !== $post_type ) {
                ...
                if (!empty($post_type_object->has_archive) && function_exists('get_post_type_archive_link')){
                    ...
                    // Add logic to have breadcrumbs by Category on Research Posts
                    if( 'research_post' == $post->post_type ){

                        $args = array( 'fields' => 'all', 'parent' => 0 );
                        $terms = wp_get_post_terms( $post->ID, 'research_category', $args );
                        if( !empty( $terms ) ){
                            $trail[] = '<span ' . $item_type_scope . '>
                                            <a href="' . get_term_link( $terms[0]->term_id, 'research_category' ) . '" title="' . esc_attr($terms[0]->name ) . '">
                                                <span itemprop="title">' . $terms[0]->name . '</span>
                                            </a>
                                        </span>';
                        }
		...
		elseif ( is_archive() ) {
			...
			if ( is_tax() || is_category() || is_tag() ) {
                ...
                // Check if $term is empty before trying to show the breadcrumbs
                if( !empty( $term ) ){
